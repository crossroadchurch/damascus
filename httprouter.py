...
###############################################################################
# Damascus - Open Source Lighting Controller                                  #
# --------------------------------------------------------------------------- #
...
class HttpRouter():
    ...
    def initialise(self):
        ...
        self.routes = [
            ...
            (r'^/channel/(.*)/(.*)$', {'function': self.channel_adjust, 'secure': False}),
            (r'^/poll$', {'function': self.poll, 'secure': False})
        ]
    ...
    def channel_adjust(self, channel, data):
        UiRegistry().getWindow().adjustChannel(channel, data)
        self.do_json_header()
        return json.dumps({'results': {'success': True}}).encode()

    def poll(self):
        channel_data = UiRegistry().getWindow().poll()
        self.do_json_header()
        return json.dumps(channel_data).encode()
